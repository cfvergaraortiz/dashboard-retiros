...
# ─────────────────────────────────────────────
# DESCARGA Y CACHÉ DEL ARCHIVO EN DISCO
# ─────────────────────────────────────────────
@st.cache_resource(show_spinner="Descargando datos (primera vez, ~1 min)…")
def get_parquet_path():
    """Descarga el parquet una sola vez y lo guarda en disco temporal."""
...
def semestre_label(mes: int) -> str:
...
with st.sidebar:
    ...
    retiros = query("SELECT DISTINCT Retiro FROM read_parquet('__PATH__') ORDER BY 1", parquet_path)["Retiro"].tolist()
    sel_retiro = st.selectbox("Retiro", retiros)

    claves = query(f"""
        SELECT DISTINCT clave FROM read_parquet('__PATH__')
        WHERE Retiro = '{sel_retiro}' ORDER BY 1
    """, parquet_path)["clave"].tolist()
    sel_clave = st.selectbox("Clave", claves)

    st.markdown("---")
    st.caption("Datos cargados desde GitHub Releases")

# ─── Datos filtrados ───
df = query(f"""
    SELECT * FROM read_parquet('__PATH__')
    WHERE Retiro = '{sel_retiro}' AND clave = '{sel_clave}'
""", parquet_path)
...
for col_ui, sem_label in [(col_oct, "Oct–Mar"), (col_abr, "Abr–Sep")]:
    ...
    for tipo in curva["Tipo"].unique():
        ...
        fig_c.add_trace(go.Scatter(
            ...
            line=dict(color=TIPO_COLORS.get(tipo), width=2),
        ))
    fig_c.update_layout(title=dict(text=sem_label, font=dict(size=13,color="#1a3a5c")),
...
hist["Solar 08–17 MWh"] = (df[df["Hora_Mensual"].between(8,17)]
    .groupby("Clave_Año_Mes")["Medida_kWh"].sum() / 1000).reindex(hist["Clave_Año_Mes"]).values
hist["Noche 18–07 MWh"] = (df[~df["Hora_Mensual"].between(8,17)]
    .groupby("Clave_Año_Mes")["Medida_kWh"].sum() / 1000).reindex(hist["Clave_Año_Mes"]).values
hist["Energía MWh"]    = (hist["Energía_kWh"] / 1000).map("{:,.1f}".format)
hist["Potencia Máx kW"] = hist["Potencia_Max_kW"].map("{:,.2f}".format)
hist["Solar 08–17 MWh"] = hist["Solar 08–17 MWh"].map(lambda v: f"{v:,.1f}" if pd.notna(v) else "—")
hist["Noche 18–07 MWh"] = hist["Noche 18–07 MWh"].map(lambda v: f"{v:,.1f}" if pd.notna(v) else "—")
